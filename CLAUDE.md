...
npm run build       # build:css + inline CSS into index.html (run before committing)
```
...
- **`dist/output.css` is committed on purpose.** Static hosts serve the repo as-is without running `npm install`, so after any change to `index.html` (new Tailwind classes) or `src/input.css`, run `npm run build` and commit the regenerated `dist/output.css` + `index.html` (which now carries the inlined CSS) together.
- **Inlined CSS:** `index.html` loads styles from an inline `<style data-inline-css>` block generated by `scripts/inline-css.js` — never hand-edit it (CI verifies freshness). `404.html` still links `dist/output.css?v=N`; bump N when the CSS changes meaningfully.
- `robots.txt` and `sitemap.xml` point at the production domain `https://rabbive.dev`.
- **Fonts are self-hosted** in `fonts/` — Inter woff2 subset to the site's used glyphs (pyftsubset), 400/500 only; nothing on the site uses 600. `@font-face` rules live in `src/input.css`; `index.html` preloads both. Do not re-add Google Fonts links; re-add the 600 weight if semibold/bold is ever introduced.
